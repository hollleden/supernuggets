...
export function BottomNav({ isDarkMode, onToggleDarkMode, onResurface }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-card border-t-2 border-foreground flex items-center gap-1 px-2 py-1.5 md:hidden pb-[calc(env(safe-area-inset-bottom)+0.375rem)]">
      <BottomItem
        href={homeHref}
...
function BottomItem({ icon, label, isActive, onClick, href }: BottomItemProps) {
  const className = cn(
    'flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-[10px] transition-colors',
    'font-mono text-[9px] font-bold tracking-widest',
    isActive
      ? 'bg-foreground text-background'
      : 'text-foreground active:bg-foreground/10'
  )
